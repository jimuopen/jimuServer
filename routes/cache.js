...
/* GET users listing. */
router.get('/', function(req, res, next) { //http://localhost:3000/get?key=adv
  //res.send('respond with a cache');
  let key = req.query.key
  console.log('--------'+ key + '--------')
  db.query("select t_value from t_template where t_key='"+ key + "'",function(err,rows){
    if(err){
      res.send({});
    }else {
      res.send(rows[0].t_value);
    }
  });
});
...
router.get("/query",function(req,res,next){
  db.query("select * from t_template",function(err,rows){
    if(err){
